    ...
    <div class="card">
        <div class="card-header">
            <h5><b>Order Details</b>
                <a href="{{ route('admin.status.order.list', 1) }}" class="btn btn-sm btn-primary" style="float: right">
                    <i class="bx bx-arrow-back"></i> Back
                </a>
            </h5>
        </div>
        <div class="card-body">
            <form id="orderUpdateForm" action="{{ route('admin.order.update', $order->order_slug) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-4">
    ...
    <div class="modal fade" id="AddOrderProduct" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.order.add.order.item') }}" method="POST">
                    @csrf
    ...
    <script>
        ...
        $(document).ready(function () {
            $('.selectOrderProduct').on('change', function () {
                ...
                if (product_id) {
                    $.ajax({
                        url: "{{ url('admin/order/add-order-item') }}/" + product_id,
                        type: "GET",
                        dataType: "json",
        ...
        $(document).ready(function() {
            $(".orderQuantityButton").on("click", function(e) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{ route('admin.order.product.quantity.update') }}",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "detail_id": detail_id,
                        "quantity": quantity,
