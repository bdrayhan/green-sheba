...
<div class="row d-flex justify-content-around">
    @foreach (deliveryPageStatus() as $row)
    <div class="col-md-2">
                    ...
                    <tbody>
                        ...
                        <form action="#" id="assingUserSubmit" method="POST">
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <button
                                        class="btn btn-sm btn-secondary">{{ $order->courier->courier_name }}</button>
                                </td>
                                <td>
                                ...
                                <td class="text-center">
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="defaultDropdown" style="">
                                            @if ($order->order_status == 8 || $order->order_status == 11)
                                                @foreach (deliveryOrderStatus() as $row)
                                                <li>
                                                    <a class="dropdown-item orderStatus" href="#"
                                                        data-order="{{ $order->id }}"
                                                        data-courier="{{ empty($order->courier_id) ? 0 : $order->courier_id }}"
                                                        data-status="{{ $row->id }}">{{ $row->os_name }}</a>
                                                </li>
                                                @endforeach
                                    ...
                                    @else
                                    <button class="btn btn-sm btn-secondary">{{ $order->status->os_name }}</button>
...
<script>
    $(document).ready(function () {
        $('.orderCourier').click(function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.order.courier.assign') }}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id": courier,
                    "order_slug": order
